CREATE OR REPLACE FUNCTION adempiere.BomQtyOnHandASIByLocator(product_id numeric, attributesetinstance_id numeric, locator_id numeric)
 RETURNS numeric
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
	...
	ELSIF (v_IsStocked='Y') THEN
		SELECT COALESCE(SUM(QtyOnHand), 0) INTO v_ProductQty
		FROM M_STORAGE s
	...
	FOR bom IN 
		SELECT bl.M_Product_ID AS M_ProductBOM_ID
			, CASE WHEN bl.IsQtyPercentage = 'N' THEN
					bl.QtyBOM 
				ELSE bl.QtyBatch / 100
			END AS BomQty
			, p.IsBOM
			, p.IsStocked
			, p.ProductType
	...
	LOOP
		...
		ELSIF (bom.IsBOM = 'Y') THEN
			v_ProductQty := BomQtyOnHandASIByLocator (bom.M_ProductBOM_ID, AttributeSetInstance_ID, Locator_ID);
			IF (v_ProductQty < v_Quantity) THEN
				v_Quantity := v_ProductQty;
			END IF;
		END IF;
